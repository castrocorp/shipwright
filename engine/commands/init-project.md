...
## Step 6: Detect MCP Integrations

Try to detect available MCP servers. Two sources are possible:
...
4. If not available, leave placeholders
...
4. If available, ask user which channel to use for task tracking
5. If not available, leave placeholders

### Notion

Probe: try a lightweight Notion call (e.g., `search`).
- If available, note the tool name for documentation access
- If not available, skip — Notion is optional

## Step 7: Detect Existing Agents
...
## Step 9: Check for Existing Stack Adapter
...
If MCP integrations were detected via **Claude.ai managed** (OAuth browser login), no `.mcp.json` is needed — report that managed integrations are active.

If integrations were NOT detected (no managed MCPs available):

1. Check if `.mcp.json` already exists. If yes, ask: update or skip.
2. If it does NOT exist, ask the user:
   ```
   No MCP integrations detected. Would you like to:
   a) Generate .mcp.json for local MCP servers (requires API tokens)
   b) Skip — you can connect integrations later via /mcp in Claude Code
   ```
3. If user chooses (a), copy `template/mcp.json` to the project root and remind about env vars.
4. If user chooses (b), skip and leave placeholders in `project.md`.
...
---
name: project-config
description: Project-specific configuration for Shipwright. Auto-generated by /init-project.
---
